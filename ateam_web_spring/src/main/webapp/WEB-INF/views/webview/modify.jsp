...
<c:if test="${vo.board_gp eq 1}">
	<h3>공지글 수정</h3>
</c:if>
<c:if test="${vo.board_gp eq 2}">
	<h3>질문글 수정</h3>
</c:if>
<c:if test="${vo.board_gp eq 3}">
	<h3>게시글 수정</h3>
</c:if>


<form method="post" action='update.ap' enctype="multipart/form-data">
<table>
...
function sendFile(file, editor) {
	...
	$.ajax({
		data : data,
		type : "POST",
		url : "qnaUploadSummernoteImageFile",
		contentType : false,
		processData : false,
